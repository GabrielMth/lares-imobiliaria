...
export class PropertyDetailComponent implements OnInit {
  ...
  private setMetaTags(p: Property) {
    ...
    // palavras-chave relevantes geradas a partir dos dados do imóvel
    const keywords = [
      tipo, status, bairro, cidade,
      `${tipo} ${status}`,
      `${tipo} ${status} ${cidade}`,
      `${tipo} ${status} ${bairro}`,
      `imóvel ${status} ${cidade}`,
      'imobiliária', 'Barrios Imobiliária',
      `${p.quartos ? p.quartos + ' quartos' : ''}`,
      `${p.areaTotal ? p.areaTotal + 'm²' : ''}`,
    ].filter(Boolean).join(', ');

    const imagem = p.urlsFotos?.length
      ? `${this.apiUrl}/uploads/${p.urlsFotos[0]}`
      : '';
  ...
  getImageUrl(fileName: string, size: 'thumb' | 'medium' | 'full' = 'full'): string {
    if (!fileName) return '';
    const prefix = size === 'thumb' ? 'thumb_' : size === 'medium' ? 'medium_' : '';
